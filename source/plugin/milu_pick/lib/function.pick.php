...
function test_window($get_type, $url_test, $is_fiter, $rules, $replace_rules, $filter_data,$show_type, $login_cookie, $filter_html_arr, $reply_is_extend){
	...
	if($get_type == 1){//dom
		if($show_type == 'reply'){
			$result_data = dom_get_manytext($contents, $rules, 'reply', 0, $reply_is_extend);
		}else{
			if($show_type == 'title'){
