...
function article_edit(){
	...
	if($_GET['submit']){
		...
		$setarr['title'] =  getstr(trim($setarr['title']), $pick_config['title_length'], 1, 1);
		if(strlen($setarr['title']) < 1) {
			cpmsg_error(milu_lang('title_no_empty'));
		}
		
		if(empty($setarr['summary'])) $setarr['summary'] = portalcp_get_summary(stripslashes($_POST['message']));
		...
		if($_GET['public_flag']){
			...
			if($article_arr['public_time'] > $_G['timestamp'] && $is_timing == 1 && $aid){
				...
				cpmsg(milu_lang('article_public_timming', str_replace(':', '-', array('d' => dgmdate($article_arr['public_time'], 'u')))).$return_list_html, PICK_GO.'picker_manage&myfunc=article_edit&aid='.$aid.'&pid='.$pid, 'succeed');
				return;
			}
			...
			if($select == 1){//门户
				...
				$old_arr['catid']  = intval($_POST['old_blog_big_cid']);
				$old_arr['classid']  = intval($_POST['old_blog_small_cid']);
				$old_arr['uid']  = intval($_POST['old_uid']);
				$old_arr['username']  = $_POST['old_username'];
				$old_arr['blog_id']  = $_POST['old_blog_id'];
...
function downremotefile($arr,$action, $old_arr = array()){
	...
	if(VIP) $arr['content'] = reply_post($arr['content'], array('cookie' => $arr['cookie'], 'page_url' => $arr['page_url'], 'cid' => $arr['cid']));
	$arr['content'] = str_replace(array("src=\r\n\"", "src=\r\"", "src=\n\"", "img\r\n", "img\r", "img\n"), array("src=\"", "src=\"", "src=\"", "img ", "img ", "img "), $arr['content']);//有些特殊的
...
function article_move_forums($arr, $old_arr){
	...
	$thread_htmlon = $_G['pick_htmlon'] = $_G['cache']['evn_milu_pick']['pick_config']['thread_htmlon'] ? 1 : 0;
	...
	$message = $arr['content'];
	if($thread_htmlon != 1) {
		$message = pick_html2bbcode($message);
	...
	if($info['tid']){//更新
		...
		if($arr['is_public_reply'] == 1 && $arr['reply'] || $arr['is_content_reply'] == 1){//是否开启发布回复
			...
			$reply_htmlon = $_G['pick_htmlon'] = $_G['cache']['evn_milu_pick']['pick_config']['reply_htmlon'] ? 1 : 0;
			foreach((array)$reply_arr as $k => $v){
				$message = dstripslashes($v['content']);
				$message = media_htmlbbcode($message, $arr['page_url']);
				$message = audio_htmlbbcode($message, $arr['page_url']);
...
function forum_article_content($arr){
	...
	$setarr['message'] = dstripslashes(clear_ad_html($arr['message']));
	$htmlon_type = $arr['is_post'] == 1 ? 'reply_htmlon' : 'thread_htmlon';
	$htmlon = $_G['pick_htmlon'] = $_G['cache']['evn_milu_pick']['pick_config'][$htmlon_type] ? 1 : 0;
	$setarr['message'] = media_htmlbbcode($setarr['message']);
	if($htmlon != 1) {
		$setarr['message'] = pick_html2bbcode($setarr['message']);
		$setarr['message'] = htmlspecialchars_decode($setarr['message'], ENT_QUOTES);
		$setarr['message'] = format_html($setarr['message']);
		$setarr['message'] = unhtmlentities($setarr['message']);
	}
	if(DISCUZ_VERSION != 'X2'){//2.5版本 
		$setarr = dstripslashes($setarr);
	}else{
...
function article_thumb($aid){
	...
	$no_thumb_mode = DISCUZ_VERSION != 'X2.5' && DISCUZ_VERSION != 'X2' && !empty($_G['setting']['portalarticleimgthumbclosed']) ? 1 : 0;
	$x3_sql = $no_thumb_mode ? '' : 'AND thumb=1';//x3以上不管是否开启缩略图都自动生成封面
	$rs = DB::fetch_first("SELECT * FROM ".DB::table('portal_attachment')." WHERE isimage=1 $x3_sql AND aid='$aid' ORDER BY dateline ASC");
	if($rs){
		$arr['pic'] = 'portal/'.$rs['attachment'];
		$arr['thumb'] = $no_thumb_mode ? 0 : 1; 
		$arr['remote'] = $remote;
...
function article_move_portal($arr, $old_arr){
	...
	$arr['title'] = getstr(trim($arr['title']), $pick_config['title_length'], 1, 1);
	if($arr['check']){
	...
	if(DISCUZ_VERSION != 'X2' && DISCUZ_VERSION != 'X2.5'){
		$pre_next_arr = portalcp_article_pre_next($arr['catid'], $aid);
...
function pick_blog_post($POST, $olds=array()) {
	global $_G, $space,$pick_config;
	$__G = $_G;
	$_G['uid'] = $POST['uid'];
	
	$_G['username'] = addslashes($POST['username']);

	$POST['subject'] = getstr(trim($POST['subject']), $pick_config['title_length'], 1, 1);
...
function forum_downremotefile($arr, $old_arr){
	...
	$arr['message'] = str_replace(array("src=\r\n\"", "src=\r\"", "src=\n\"", "img\r\n", "img\r", "img\n"), array("src=\"", "src=\"", "src=\"", "img ", "img ", "img "), $arr['message']);//有些特殊的
	...
	$temp = $temp ? $temp : array();
	$temp = array_merge($temp, $attach_arr);
	if(VIP) $arr['message'] = reply_post($arr['message'], array('cookie' => $arr['cookie'], 'page_url' => $arr['page_url'], 'cid' => $arr['cid']));
	$del_a = 0;
	if($arr['content_filter_html'][0] == 0 && $arr['content_filter_html']){
		$del_a = 1;
	}
		...
		foreach($temp as $key => $value) {
			...
			if(strlen($imageurl)) {
				...
				if(!isset($existentimg[$hash])) {
					...
					if(preg_match('/^(http:\/\/|\.)/i', $imageurl)) {
						if($imageurl && snoop_obj) $content_re = get_img_content($imageurl, $snoop_obj, $ext, array('referer' => $arr['page_url']));
						if(is_array($content_re)){
							$content = $content_re['content'];
							$file_name = $attach['name'] = $content_re['file_name'] ? $content_re['file_name'] : ( $value[2] ? _striptext($value[2]) : time().'.'.$content_re['file_ext']);
							$attach['ext'] = $content_re['file_ext'] ? $content_re['file_ext'] : trim($upload->fileext($file_name));
