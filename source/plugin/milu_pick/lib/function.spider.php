...
function get_img_content($img_url, $snoopy_obj = '', $ext = '', $args = array()){
	global $_G;
	...
	if(!function_exists('fsockopen') && !function_exists('pfsockopen') || !$snoopy_obj){
		...
		if($header_arr['content-disposition']){
			...
			if(!$info['file_ext']){
				...
				//基于文件头获取扩展名
				$info['file_ext'] = $info['file_ext']  ? $info['file_ext'] : FileExt::get_fileext($content);
			}
		}
		if($info['file_ext']){
...
//dom获取多个内容段
function dom_get_manytext($content, $dom_rules, $filter_type = 'reply', $count = 0, $reply_is_extend = 0){
	...
	if($filter_type == 'reply' && $reply_is_extend == 1) unset($text_arr[0]);//如果回复是继承，删除第一个结果
	$html->clear();
	unset($html);
	return $text_arr;
