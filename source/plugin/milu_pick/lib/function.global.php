...
function str_format_time($timestamp = '', $format_str = ''){  
	...
	$timestamp = mb_ereg_replace('^(　| )+', '', $timestamp);
	$timestamp = mb_ereg_replace('(　| )+$', '', $timestamp);//去掉首尾空格
	if($format_str){
...
function media_htmlbbcode($text, $url= '', $type = 'bbs'){
	...
	if(!$text) return;
	$text = dstripslashes($text);
	if($_G['pick_htmlon'] == 1){//开启html
		preg_match_all('/(src)=(["\'])?([^\'" ]*)(?(2)\2)/i', $text, $matches);
		$audio_url = _expandlinks($matches[3][0], $url);
		$replace_url = str_replace($matches[3][0], $audio_url, $matches[0][0]);	
...
function audio_htmlbbcode($text, $url = '', $type = 'bbs'){
	global $_G;
	if($_G['pick_htmlon'] == 1){//开启html
		preg_match_all('/<param\sname="url"\svalue="(.*?)"/is', stripslashes($text), $matches, PREG_SET_ORDER);
		if(is_array($matches[0])) {
			$audio_url = _expandlinks($matches[0][1], $url);
		}
...
function get_audio_param($attributes, $page_url, $type = 'bbs') {
	...
	return $type == 'bbs' ? '[media='.$x.','.$value['width'].','.$value['height'].']'.$audio_url.'[/media]' : '[flash=mp3]'.$audio_url.'[/flash]';
...
if (!function_exists('json_decode')){
	function json_decode($s, $ass = false){
		...
		$assoc = ($ass) ? 16 : 32;
		$gloJSON = new Services_JSON($assoc);
