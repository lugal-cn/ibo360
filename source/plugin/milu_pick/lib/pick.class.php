...
class pick{
	...
	function robot($level){
		...
		foreach((array)$this->now_url_arr as $k => $url){
			...
			if($visit_flag > 0 ){
				...
				if($this->now_level > 1 && $next_link) $this->pick_cache_data['now_level'] -= 1;
				if(!$this->flip()) return;
				if($this->now_level > 1 && $next_link) $this->pick_cache_data['now_level'] += 1;
	...
	function get_reply($content, $reply_num = 0){
		...
		if($this->p_arr['reply_get_type'] == 1){
			$reply_arr = dom_get_manytext($content, $this->p_arr['reply_rules'], $type , $reply_num, $this->p_arr['reply_is_extend']);
		}else{
			$reply_rules = str_replace('[body]', '[reply]', $this->p_arr['reply_rules']);
			$reply_arr = str_get_str($content, $reply_rules, 'reply', -1);
			if($type == 'reply' && $this->p_arr['reply_is_extend']) unset($reply_arr[0]);
	...
	function get_article($content,$url = ''){
		...
		//需要回复才能看到的内容
		if(VIP) $article_info['content'] = reply_post($article_info['content'], array('cookie' => $this->p_arr['login_cookie'], 'page_url' => $url));
	...
	function format_article($article_info){
		...
		$article_info['title'] = getstr(trim($article_info['title']), $this->pick_config['pick_config']['title_length'], 1, 0, 0, 1);
	...
	function article_public($arr){
		...
		$arr['article_tag'] = $this->plugin_set['open_tag'] == 1 && $this->p_arr['public_type'] != 1  && empty($arr['article_tag']) ? implode(',', dz_get_tag($arr['title'], $arr['content'], 1)) : $arr['article_tag'];
		...
		if($insert_aid){
			...
			if($this->p_arr['public_type'] == 2){//论坛
				$arr['cookie'] =  $this->p_arr['login_cookie'];
				if($arr['is_download_img'] == 1 || $arr['is_download_file']) forum_downremotefile($arr, $old_arr);
			}else{
