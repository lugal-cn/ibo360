...
class plugin_milu_pick {
	...
	function global_header() {
		...
		if( ( $set['cron_open'] == 1 || $set['is_timing'] == 1 ) && strexists(strtolower($_G['setting']['plugins']['version']['milu_pick']), 'vip')){
			...
			if($set['cron_open'] == 1) {
				loadcache('pick_run');
				if(intval($_G['cache']['pick_run']) <= TIMESTAMP) pick_cron::run();
			}
			if($set['is_timing'] == 1) {
				loadcache('pick_timing');
				if(intval($_G['cache']['pick_timing']) <= TIMESTAMP) pick_cron::run_timing();
			}	
	...
	function _public_add_info($type = 'bbs'){
		...
		if($type == 'bbs'){
			...
			$info['content'] = htmlspecialchars_decode(pick_html2bbcode($info['content']));
	...
	function viewthread_bottom_output(){
		...
		foreach($postlist as $pid => $post) {
			if($post['first'] == 1){
				...
				$thread_title = cutstr($seo_arr['title'], 52);
